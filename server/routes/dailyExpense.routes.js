...
router.patch("/:expenseId", auth, async (req, res) => {
  try {
    ...
    if (expenseId) {
      const rates = await Rate.find({ id: req.params._id });
      const dailyExpense = await DailyExpense.findById(expenseId);
      const filterRates = rates.filter(
        (el) => dailyExpense.data.includes(el.data) && el._id
      );
      const updatedDailyExpense = await DailyExpense.findByIdAndUpdate(
        expenseId,
        {
          ...req.body,
          quantityRate: filterRates.map((el) => el._id),
        }
      );
